...
class SortedAmountSpentByCustomer(MRJob):
    ...
    def reducer_get_sum(self, customer_id, values):
        # Sum the values and emit the result
        yield customer_id, sum(values)

    def combiner(self, customer_id, values):
        # Combine the values locally before sending to the reducer
        yield customer_id, sum(values)

    '''
    '%04.2fd' % float(total) == 0000.00 % total
    if total is 1234.56 the result becomes 1234.56
    since the sort of a MapReduce job is lexicographical, 
    this will sort the results by the total amount spent
    being that the highest value is 6375.45 therefore the use 
    of 04.2f will ensure that the result is sorted correctly
    '''
